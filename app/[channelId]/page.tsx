...
import { Badge as TwitchBadge, User as TwitchUser } from '../twitch/types';
...
export default async function ChazzyPage({
  ...
  const twitchBroadcasterId: string = await fetch(`https://api.twitch.tv/helix/users?login=${twitchChannelId}`, {
    headers: {
  ...
  const globalBadges = await fetch('https://api.twitch.tv/helix/chat/badges/global', {
    headers: {
      'Content-Type': 'application/json',
      Authorization: `Bearer ${twitchAccessToken}`,
      'Client-Id': twitchClientId,
    ...
    .then((r) => r.json() as Promise<{ data: TwitchBadge[] }>)
    .then((data) => data['data'] ?? []);
